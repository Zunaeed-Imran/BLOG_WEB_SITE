...
$result = $conn->query("SELECT * from `blog` WHERE `id` = {$_GET['id']} LIMIT 1;");
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
  ...
  if (move_uploaded_file($_FILES['image']['tmp_name'], $image) && $conn->query("UPDATE `blog` SET `image` = '{$image}', `name` = '{$_POST['name']}', `about` = '{$_POST['about']}' WHERE `id` = {$_GET['id']};")) {
    header('location: ./');
  }
}
...
<body class="p-3 mb-2 bg-dark text-white">
  ...
  <form action="edit.php?id=<?= $row['id'] ?>" enctype="multipart/form-data" method="post">
    ...
    <label>
      <span>About: </span>
      <input name="about" type="text" value="<?= $row['about'] ?>" />
    </label>
